...
class Cookies {
    ...
    public function reviveSessions() {
        ...
        if($config['sessions']['user_cookie']['enable']) {
            # Obtener
            $user_session = $session->get($this->get('session_hash') . $config['sessions']['unique'] . '__user_id');
            ...
            if(null == $user_session && null != $salt && null != $encrypt && Helper\Strings::chash($salt ?? '_', $encrypt ?? '/$')) {
                ...
                $session->set($this->get('session_hash') . $config['sessions']['unique'] . '__user_id',(int) $id_user);
            }
        }       
    }
}
